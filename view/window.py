...
class Window(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        icon_photo: tk.PhotoImage = tk.PhotoImage(
            file=resource_path(relative_icon_path)
        )
        ...
        self.__save_button: tk.Button = tk.Button(
            self.__button_frame, text="Save", command=self.__text_field.save_input
        )
        self.__load_button: tk.Button = tk.Button(
            self.__button_frame, text="Load", command=self.__text_field.load_saved_input
        )
        self.__clear_button: tk.Button = tk.Button(
            self.__button_frame, text="Clear", command=self.__text_field.clear_text
        )
    ...
    def __create_tooltips(self) -> None:
        ...
        self.__tooltip_for_save_button: ToolTip = ToolTip(
            self.__save_button, "Saves text - CTRL+S"
        )
        self.__tooltip_for_load_button: ToolTip = ToolTip(
            self.__load_button, "Loads text - CTRL+L"
        )
        self.__tooltip_for_clear_button: ToolTip = ToolTip(
            self.__clear_button, "Clears text - ALT+C"
        )
